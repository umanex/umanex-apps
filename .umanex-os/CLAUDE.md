...
**De uitleg volgt de afstand, niet mijn kennis.** *"Jeroen heeft de context al"* geldt voor het werk dat hij zag — niet voor lagen die ik er zelf onder heb aangebracht. Ben ik tijdens een taak dieper gegaan dan de vraag reikte (metingen, interne werking, een uitstapje van drie tool-calls), dan kom ik in de samenvatting **eerst terug naar boven**: waar ging dit over, wat is er af, wat vraagt een beslissing. Herkenningsteken, en het is hard: sta ik op het punt een keuzevraag te stellen waarin termen voorkomen die pas tijdens míjn werk zijn ontstaan, dan is die vraag nog niet stelbaar — leg eerst uit in de taal van vóór het uitstapje. Gemeten 2026-09-08: een `AskUserQuestion` over `spoor.py`, vuurfrequenties en beslistakken kreeg als antwoord *"ik ben het overzicht kwijt"*; geen van die drie termen bestond aan het begin van de opdracht.

**Lengte:** Antwoord + argumentatie + relevante caveats, dan stoppen. Geen volledige uitwerking tenzij gevraagd. Dit is geen stijlregel maar de grootste wachttijdpost: gemeten 2026-09-18 over een sessie van 120 minuten was **70% mijn eigen generatietijd** — geen tools, geen CI, geen tests. Elke ongevraagde tabel en elk uitstapje naar een aangrenzend probleem is tijd waarin Jeroen wacht. Conclusie eerst, bewijs erbij, tabellen alleen op verzoek; een aangrenzend probleem noem je in één zin en bouw je pas na een ja.
...
**De rollen in één regel** — PLAN levert de acceptatie-checklist (TC-EBC bij design, **main-agent only**; een licht taak-contract bij refactor/bugfix/infra) — en bij **afhankelijke berekeningen** ook minstens één invariant over het hele model, want scherm-per-scherm valideert daar niets — BOUW draait in de main-agent of de bouw-skill bij het taaktype, BEOORDEEL is een panel van `code-review` · `verify` · `ux-audit` · `security-audit` met de main-agent als scheidsrechter, en draait als **één run** via `scripts/checklijst.sh` — niet als een losse probe per item. Die runner weigert een groep zonder tegenproef (exit 2) en print de afvinkregels mét bewijs; het waarom en de gemeten kost staan in de `cyclus-tot-validatie` skill. Produceert een meet-, parity- of review-ronde zélf bevindingen (F1…F5) — ook midden in een bouw-skill, ook in iteratie twee — dan is díe lijst de bron van de checklist: elk gemeten item wordt één-op-één een acceptatie-item **vóór je er één van fixt**, en een bevinding met twee wijzigingen in één regel wordt twee items. "kpi-strip: add top border, remove `py-1.5`" leest als één ding, dus de helft die wegviel viel stil weg en de ronde sloot af met "nul verschillen" terwijl de scheidingslijnen de haarlijnen niet raakten. De **`cyclus-tot-validatie` skill** heeft de mapping, het panel per as en de reviewer-discipline — lees hem voor je de lus start.
...
**En een uitkomst die je instrument produceert maar die niemand leest, bestaat niet.** Elke guard hierboven levert één bit (de exit-code) en een rijkdom aan tekst voor een lezer — en als de lus geen lezer heeft, is die tekst stilte met een teller. Gemeten op rowtrack (2026-09-10), drie keer in één ronde: een beeld-as die van 74,36 naar 56,18 daalde zonder dat iemand wist wat er in die 56,18 zat; 31 meldingen "Figma kan deze marge niet uitdrukken", geschreven en nooit gelezen; en 23 machinaal afgeleide property-namen in een gepubliceerde library die geen ontwerper ooit zag. Drie vormen, drie plichten: **een aggregaat als bewijs draagt zijn ontleding** (het grootste onderdeel, of de som per oorzaak — anders is een daling een richting zonder vloer) · **een meldingsoort draagt een ratel**, tweezijdig, zodat een vermelding erbij rood is en niet stil · **een gegenereerde naam in een gedeeld oppervlak passeert een naamlijst** vóór hij gepubliceerd wordt. Herkenningsteken: een instrument dat een lijst print en exit 0 geeft. De uitwerking staat in `verify` rail 15.
...
*Nooit een destructief pad tegen productiedata.* Dit begrenst de vorige regel. Toets de guard in plaats van het effect, draai de logica op synthetische invoer, of gebruik een testaccount. Lukt geen van drieën: `[NIET TE VERIFIËREN — destructief pad, geen testaccount]`. Dat is een leemte, geen vrijbrief; wil je het écht uitvoeren, vraag het vooraf.

*Een instrument dat draait is nog geen instrument dat meet.* Toon vooraf dat de uitkomst kón afwijken: een **guard** op béide kanten, een **groene** uitkomst een negatieve, een **vervangen** instrument het óngewijzigde, een **lege, stille of nul**-uitkomst een positieve — nul is even vaak wáár als kapot: een teller draagt zijn **noemer**, stilte een tweede kanaal. Alléén bevestigen kan een **format dat het antwoord inbakt** of een **zwarte lijst voor een positieve regel**. Beoordelaars op één afgeleide bron: één meting. Anders `[NIET TE VERIFIËREN — reden]`, geen proxy.
...
*Een lus over een variabele draait in zsh één keer.* De tool-shell is zsh: `for f in $VAR` itereert zonder fout over één lange string, en een glob zonder treffer breekt de hele aanroep af. Gebruik:

```bash
printf '%s\n' "$VAR" | while IFS= read -r f; do … ; done
setopt nullglob   # per aanroep; (N) werkt hier niet
```

of `xargs`. Committed `#!/bin/bash`-scripts zijn correct; dit geldt inline. Een lege uitkomst vraagt eerst of de lus gedraaid heeft. De shell houdt zijn **cwd** vast tussen calls — anker met `git -C <repo>` of absolute paden.
...
*Een muterende stap is zelf een meting.* Opruimen, resetten, patchen, stashen, afkappen — een stap die de toestand verandert vóór je meet en die per constructie niet kán klagen, is geen handeling maar een aanname. `rm -rf` op een niet-bestaand pad geeft exit 0; een patch die zijn doelregel mist meldt groen. Eis een uitkomst op het effect: `[ -d "$PAD" ] || { echo "STOP — pad bestaat niet"; exit 1; }`, of voor een patch `cmp -s "$M" "$M.bak" && { echo "STOP — raakte niets"; exit 1; }` — `git diff` is daar blind voor een ongetrackt bestand, en het herstel hoort in een `trap`.

*Een naam is een bewering over het ding, niet het ding.* Identificeer aan **inhoud**, niet aan label, tag of type — en tel eerst: `querySelectorAll(…).length` hoort **1** te zijn. Spreken twee onafhankelijke signalen elkaar tegen, dan is dat het **alarm**: verklaar het of meld beide, in dezelfde aanroep. Trek een onbevestigde gevolgtrekking nooit door naar zusters. Een rail beschermt wie hem leest, niet het script dat hij schreef: een procedure die je committeert telt zélf, en een filter dat je schrijft (glob, `case`, selector) toets je op een naamgenoot zonder die inhoud.

*Het meetbereik bevat vaak de meting zelf.* Een `grep -c` op een bestand met zijn eigen format-voorbeeld telt dat mee; een CI-log bevat het script dat hij logt; `$?` na een pipe geeft de status van de láátste pijpcomponent. Anker een telling op het **object**, filter de bron uit de log, en lees een exit-status vóór de pipe: `out=$(cmd); rc=$?`. Bij een bestandsgebaseerde scope: `git status --porcelain` rapporteert een nieuwe map als één regel — gebruik `-uall`, naast `git diff --name-only origin/main...HEAD`.

*Een grep-treffer is niet de entry.* De `Status` van een lus-item staat buiten het venster van een kale `grep -n`. Een groter venster is een nieuwe proxy; anker op de entry:

```bash
awk -v s=open '/^## [0-9]{4}-[0-9]{2}-[0-9]{2}/{h=$0; next}
  /^- \*\*Status:\*\*/{if(h!=""){if($3==s)print h; h=""}}' <bestand>
```

*Een verwachtingswaarde is geen meting.* Een vuistregel, typische waarde of logisch ogend aggregaat is een **hypothese**, nooit het bewijs dat een vraag afsluit — een telling op de **invoer** evenmin, tot je de bewerking verrekent, en de **herinnering van de gebruiker** evenmin: kun je het in één call meten, meet dan en vraag hoogstens ter bevestiging. Het schaadt het meest in een **keuzevraag**: een ongemeten getal of geruststellende zin wordt in een `AskUserQuestion`-optie de grond van de beslissing. Zet daar je meting of markeer het als schatting; kantelt de aanname, vraag opnieuw.
...
**Brug naar de eval-loop.** Een gefaalde review die een **terugkerende faalklasse** blootlegt = een `vastleggen`-trigger. De triade is de *feeder* van de trage loop, geen duplicaat. Houd de assen uit elkaar: triade-status (`gepland → gebouwd → gevalideerd`, per taak) staat los van learning-status (`open → verified → promoted`, of `closed` wanneer er niets te harden valt, over sessies heen).
...
**Toets een bewering over een bibliotheek aan de geïnstalleerde bron.** Die staat in `node_modules` — lees hem vóór je een fix bouwt op wat een API "zou moeten" doen. Hoe stelliger de bewering, hoe kleiner de kans dat ze nagekeken is, en een typecheck die slaagt zegt niets over een verkeerd begrepen contract. Dezelfde vorm als de DTCG-valkuil verderop. Dit geldt net zo goed voor je **meetgereedschap**, waar het erger uitpakt: een verkeerd contract in een fix faalt zichtbaar, in een instrument levert het een gevulde, geloofwaardige, verkeerde uitkomst — en een onbekende optie wordt stil genegeerd, in een `.mjs` zonder typecheck die hem afkeurt. Herkenningsteken: een check die eerst groen was en na een tussenliggende meting rood wordt zonder dat de code veranderde, is een uitspraak over het instrument, niet over de code.
...
**Verander één ding tegelijk als je een oorzaak zoekt.** Herschrijf je een gefaalde call en werkt de nieuwe versie, dan heb je de oorzaak alleen gevonden als er één verschil was. Gemeten op 2026-08-26: drie wijzigingen in één herschrijving (`·` eruit, `children.filter` → `findAll`, arrow → `function`) bewezen niets over welke het deed — en de aangewezen verdachte bleek onschuldig. **Dezelfde fout in de spiegel: twee wáárnemingen die in meer dan één ding verschillen.** Bij een eigen herschrijving weet je dát je drie dingen veranderde; bij twee gevonden waarnemingen kun je de verschillen niet eens opsommen — dus tel ze eerst, en concludeer niets zolang het er meer dan één is. Gemeten op 2026-09-08: een groene CI-run op `main` (11:18) naast een rode op een feature branch (11:28) verschilde in **commit, machine én tijdstip**, en ik hing de conclusie aan het enige verschil dat ik kende — mijn wijziging. `gh run rerun <id> --failed` op dezelfde commit gaf 18/18: de test was flaky. Herkenningsteken: *"het werkte daar wel en hier niet, dus het komt door X"* waarbij X het enige verschil is dat je toevallig kent. De goedkope controle is meestal één commando dat één variabele vastzet.

**Een boolean die uit een object komt, is per constructie waar.** `rechts ? 1 : -1`, waar `rechts` de node is die het rechterpaneel draagt: bij de koude start is dat de lege-toestand-node — een object, dus truthy — waardoor het scherm dat "nog geen PV gekozen" heet een geselecteerde rij toonde. Geef de conditie een eigen expliciete parameter (`selIndex`) in plaats van hem uit een naburig object af te leiden. En let op bij een **herbouw**: ik patchte de instantie, herbouwde daarna de sectie omdat de maten niet klopten, en kopieerde dezelfde afleiding onveranderd mee — dezelfde bug een tweede keer in hetzelfde scherm. Een herbouw is de plek om de oorzaak te vervangen, niet om hem over te tikken.
...
**Schrijf een oorzaak pas op als de kandidaten uitgesloten zijn.** Passen er twee verklaringen op dezelfde waarneming, dan is er meestal één goedkope test die ze scheidt — en die kost bijna altijd minder dan de gevolgen van de verkeerde opschrijven. Het duurzame document maakt het asymmetrisch: een losse verkeerde bewering corrigeer je in één zin, maar een oorzaak in een `CLAUDE.md` leest een sessie later als vaststaand, propageert naar elke sessie van die laag, en kan zichzelf niet tegenspreken. Gemeten 2026-09-10 (rowtrack): na een bevestigde publicatie gaf een import in het consumerende bestand drie van vijf properties terug; ik schreef "het bestand houdt een achterstallige spiegel, haal de update binnen in het Assets-paneel" in twee documenten. De echte oorzaak was de plugin-runtime die zijn imports cachet vanaf het verbinden, en één herstart wees dat uit. Het tegensignaal lag er al — Figma's eigen paneel had niets te updaten — en ik had het niet opgezocht vóór ik schreef. **Twee onafhankelijke signalen die elkaar tegenspreken zijn het alarm**, niet de ruis; en zolang de test niet gedaan is, schrijf je een kandidaat op mét zijn test, geen oorzaak.
...
**Een ontbrekende waarde is zelf een state, geen gat om te vullen.** Heeft de code geen waarde, dan toont ze dat — ze vult er geen plausibele in. Geen fixture als stille terugval, geen oorzaak in UI-tekst die de code niet vastgesteld heeft, en nooit tekst in een veld dat van de gebruiker is om een handeling te laten slagen: weigert de laag eronder een lege waarde, dan is niet doorgaan het juiste antwoord. Herkenningsteken: een waarde die stroomafwaarts niet meer van een echte te onderscheiden is. Gemeten in umanex-apps (`apps/jobradar`), drie keer in vijf weken en elke keer per instantie gepatcht (`1f7f72e`, `49072dc`, `76a29ff`) — de derde in een sessie waarvan de opdracht *"geen verzonnen voortgang"* letterlijk eiste.
...
als hem gebruikt krijgen: een app kan de dependency, `transpilePackages` én een CI-build dragen
en de package tóch in nul app-bestanden importeren. Zonder een sectie die zegt wat de bron ís,
valt dat verschil aan niets op — en daarom toetst `ds:guard` de adoptie als eigen as. De meting
staat in de **`tc-ebc` skill**, scaffold stap 1.
...
App-werk gebeurt in de hoofdtree, in `<repo>/apps/<app>`, op een feature branch. Niet in een zusmap `../<repo>-<app>`, niet in een permanente worktree per app. Die conventie ("één app, één worktree") stond hier tot 2026-08-25 en is geschrapt — de meting die haar sloopte, de twee gevallen waarin een tweede tree tóch mag, het bijwerken zonder losse HEAD en wat te doen met een oude zusmap staan in de **`worktree` skill**. Komt er een tweede tree in beeld, lees die eerst; hij hoort altijd **ín de repo** (`.claude/worktrees/`), nooit in `~/Documents`, en nooit permanent.
...
- **Eén taak tegelijk per repo — en dat is een poort, geen kijkje.** Drie controles vóór je vertakt: `git status --porcelain -uall` (onvastgelegd werk van een ándere taak of sessie), `git rev-parse --abbrev-ref HEAD` (staat HEAD op een feature branch die niet van jou is, dan loopt díe taak nog — een schone tree bewijst niets, de PR staat gewoon open) én **`git stash list`**. Ze staan in een **eigen aanroep** die eindigt op `|| exit`, nooit geketend aan de `checkout` zelf — dezelfde vorm als het merge-protocol verderop, dat `rc` en `state` leest vóór het opruimt:

```bash
dirty=$(git status --porcelain -uall); head=$(git rev-parse --abbrev-ref HEAD); stash=$(git stash list)
[ -z "$dirty" ] && [ "$head" = main ] && [ -z "$stash" ] \
  || { echo "STOP — tree bezet: HEAD=$head"; exit 1; }        # eigen call; de checkout komt pas hierna
```

  Twee keer op 2026-09-08 ging dit mis, van twee kanten. Eén sessie liet de controles wél draaien maar in dezelfde Bash-aanroep als de `checkout -b`: ze waarschuwden pas toen de branch er al stond, en een onvastgelegde `main.ts` reisde mee. De andere las de passage in zijn vorige vorm — *"kijk naar drie dingen"* — en sloeg hem over; er stonden nul `|| exit`-vormen in deze passage tegen één in het merge-blok. Een controle die niet vóór de actie kan stoppen is een logregel, geen controle. Die derde is de plek die de eerste twee níet tonen: een stash is werk dat niet in `status`, niet in `worktree list` en niet in `log` verschijnt, en dus onzichtbaar blijft in elke controle die daarop leunt. Let bij een oude stash op de vorm van het defect: hij hangt aan de commit waarop hij gemaakt is, dus hoe ouder hij is hoe minder zijn inhoud over vandaag zegt — toepassen is dan geen herstel maar een blinde wijziging aan een draaiende app. En een stash die blijft staan terwijl zijn inhoud óók in de tree zit, is de handtekening van `git stash apply` in plaats van `pop`. De twee gemeten gevallen (een gemiste stash naast 1 425 regels ongetrackt werk, en een aprilstash met hógere dependency-majors dan productie) staan in de **`worktree` skill**, *Tref je een oude stash aan*. In al die gevallen begin je er niet stil een tweede naast, en maak je óók niet stil een eigen tree: meld het en laat Jeroen kiezen — om beurten, eerst afronden, stashen, of een tijdelijke tree in `.claude/worktrees/<taak>` die na de merge weer weg is. Die vierde laat het andere werk onaangeroerd, maar is niet gratis: zet in de vraag wat hij in díe repo kost (install, `.env.local`, een dev-server uit de hoofdtree), en merge en ruim op volgens de `worktree` skill — in zo'n tree faalt `git checkout main`, dus het merge-blok verderop werkt er niet letterlijk. Is de tree vrij, vertak dan altijd vanaf `origin/main` en nooit vanaf de HEAD die je aantreft: `git fetch -q origin && git checkout -b <type>/<beschrijving> origin/main`. Een branch vanaf andermans HEAD erft diens commits — dat is het mechanisme van 2026-08-07, en beide hooks zwijgen erover (gemeten).
...
**Je eigen sub-agents tellen mee.** Draait er een agent die jíj gestart hebt in je eigen tree, dan is die tree niet meer alleen van jou: hij schrijft scratch-bestanden en haalt een formatter over een bestand dat jouw acceptatielijst ongemoeid verklaarde. Gemeten: zeven `.tmp-*.mjs` en een geherformatteerde `DataTable.tsx` reisden mee in één `git add -A`, en twee van de agents hadden de botsing zélf al in hun eindrapport gemeld — het signaal lag er vóór de commit, ongelezen. Stage per pad zolang er agents lopen, en lees hun rapporten vóór je commit, niet erna. Hetzelfde geldt voor een achtergrondproces dat jij startte en dat bestanden herschrijft: geven herhaalde runs zonder jouw wijziging steeds iets anders, kijk dan eerst met `git status` naar het meetobject vóór je de omgeving verdenkt. Gemeten 2026-09-15: `test-doctor.sh` gaf 319/3, 235/87 en 306/16 terwijl de eigen mutatietest op de achtergrond `doctor.sh` herschreef — na het stoppen 322/0, vier keer op rij.
...
**Wacht nooit in de voorgrond op CI.** `gh pr checks --watch` in een gewone call is stilstaan: gemeten 2026-09-18 was dat 12 van de 23 tool-minuten van een sessie, één keer zeven minuten aan één stuk. Start de watch met `run_in_background` en werk door; de notificatie komt vanzelf. Hetzelfde geldt voor een testsuite die langer dan een minuut duurt.
...
**Twee hooks, tien signalen.** `.githooks/pre-commit` weigert een commit op `main`/`master` en op een losse HEAD — de veiligheidsklep is daarmee afdwingbaar in plaats van een goed voornemen, en een linked worktree op een losse HEAD kan geen commit stil kwijtraken. Rebase, cherry-pick en een lopende merge laat hij met rust. Daarnaast *waarschuwt* hij (zonder te blokkeren) over acht dingen: onvastgelegd werk in een ándere app dan deze commit — het signaal om die andere taak eerst af te ronden of te stashen, niet om zelf een tweede tree te maken — een aangeraakte app zonder `## Verify-pad`-sectie, een aangeraakte app zonder `## Design-systeem-bron`-sectie, een nieuwe open HANDOFF-entry zonder `Check`-regel, een nieuw `- [x]` zonder `bewijs:` in een `*.tcebc.md`, een acceptatie-item dat een inhoudelijke claim aan een procedurele geniet (`… ongewijzigd; tsc groen` — die twee delen nooit een instrument), een kaal PR-nummer in tekst die naar andere repo's reist, en een aangeraakte briefing met een Status buiten `gepland → gebouwd → gevalideerd`. `.githooks/commit-msg` blokkeert een commit met een app-scope die een ándere app raakt — `fix(cashflow):` mag niet aan `apps/rowtrack/` komen. Scopes die géén app zijn (`chore:`, `feat(tokens):`, `refactor(config):`) blijven vrij: een gedeelde laag hoort in één commit met de apps die hij aanpast. Is een cross-app commit écht bedoeld, zet dan een `Cross-app: <reden>` trailer in de body — expliciet en greppable. `--no-verify` is de slechtere weg: dat slaat álle hooks over, ook de snapshot- en token-sync.
...
**En "elke" is elke call, niet alleen de eerste.** Opent een tweede bestand de plugin, dan wordt dát stil het actieve doelwit: `figma_get_status` toont twee `connectedFiles` met `isActive` op het nieuwst verbonden bestand, zonder dat de lopende sessie iets merkt. Een status-call vóór je execute dekt dat niet — tussen die twee calls kan het doelwit alsnog verschuiven. De identiteit hoort dus **in** de call: begin élke schrijvende `figma_execute` met een guard op `figma.fileKey` (niet op `figma.root.name`), óók wanneer er bij aanvang maar één bestand verbonden was — dat is precies de conditie waaronder het misgaat. Skelet: `code-naar-figma` stap 6. Gemeten 2026-09-11: 23 schrijfacties gingen uit van het bestand van bij sessiestart, en de hernoem-call die in een ander klantbestand landde viel alleen om omdat díe paginanaam daar niet bestond.
