...
## 2026-09-16 — Figma-schermbouwer: acht fix-commits in 90 dagen op dezelfde bouwketen (faalklasse uit de historie)
- **Input:** faalklasse uit de historie, geen trigger-prompt. Reproduceerbaar: `git log --since="90 days ago" --oneline -- apps/rowtrack | grep -iE 'fix\(rowtrack\).*(frame|screen|scherm|library|mirror|spacer|budget)'` — gemeten 2026-09-16 op `chore/rowtrack-figma-sync-after-review`: `fcd4592` (plaatsing per spec-index, niet per paginahistorie), `9708fd0` (spacer kost een gap, inline run heeft een rij nodig), `d3d3753` (bouw geweigerd tegen een stale library-spiegel), `fd95441` (tijdsbudget, import-warm-up, leaf houdt zijn maat), `7571740` (42 schermverschillen aan de bron), `08a0ddc` (de walker ziet de modal-portalen), `9d54aa0` en `134273b` (lege frames). De keten: `apps/rowtrack/figma/builder.js`, `apps/rowtrack/scripts/geometry-parity.mjs`, `apps/rowtrack/scripts/figma-sync-check.mjs`.
- **Fout:** Acht `fix(rowtrack)`-commits in 90 dagen zetten elk één symptoom van de schermbouw recht, en elke volgende commit bewijst dat het vorige symptoom niet het laatste was. Wat níet is vastgesteld, en wat deze entry als open vraag draagt: of één ongemeten as al die symptomen verbindt — de bouw wordt pas achteraf door parity getoetst, `d3d3753` is de enige weigering vóór de bouw — of dat het acht losse oorzaken zijn. Gezien bij `sessie-reflectie` stap 2 (faalklassen uit de historie) tijdens een `/doctor`-sessie, zonder de bouwketen zelf gelezen te hebben; de verificatie hoort bij een sessie die dat wél doet.
- **Status:** open
