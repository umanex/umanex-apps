...
*Een teller van de gevallen mét uitkomst is geen frequentie.* De spiegelvorm van de alinea hierboven: niet een nul die als "niets te doen" leest, maar een positief getal dat als "altijd" leest. GEMETEN 2026-09-16 (`/doctor`, check "slow hooks"): een transcript-scan over de 50 recentste sessies gaf *UserPromptSubmit: 220 runs, gemiddeld 582 tekens*, en ik schreef in het rapport dat `tcebc-reminder.sh` "582 tekens bij élke prompt levert, ook bij niet-designvragen zoals deze". De scan telde alleen runs met niet-lege output — dat stond letterlijk in de instructie — dus over de prompts waarop de hook zweeg kon hij niets zeggen. De tweezijdige toets drie calls later kantelde het: `printf '%s' '{"prompt":"git status tonen"}' | ~/.claude/hooks/tcebc-reminder.sh | wc -c` geeft 0, dezelfde regel met *"maak een nieuw scherm met een component"* 694. *Herkenningsteken:* "bij elke", "altijd" of "ook bij" in een zin die op een telling rust — dat is een frequentie, en die vraagt de noemer (hier: het aantal prompts), niet alleen de teller. De kern stond in `CLAUDE.md` en vuurde niet. Niet blind afgespeeld: een verse lezer die de telling in drie regels voorgeschoteld krijgt, ziet de ontbrekende noemer wél, terwijl hij in het origineel onder een scan van vijftig transcripts lag.

*Verifieer met het instrument dat oordeelt, niet met een handgetikte variant ervan.* GEMETEN 2026-09-17 (umanex/umanex-os#258): een nieuwe eval-grader kreeg `pattern: '#### F1'` — mét de apostroffen in het patroon. Bij het bouwen is hij nagekeken met een `grep` die met de hand getikt was; het instrument dat rood of groen beslist (`replay-case.sh`, dat met `gsub(/^"|"$/, "")` alleen dúbbele quotes strip) is nooit gedraaid. Tegen een echt rapport: `grep -cE "'#### F1'"` geeft **0**, zonder de apostroffen **7**. De grader kon vanaf de dag dat de case werd toegevoegd (2026-09-14) per constructie niet groen worden, en drie dagen lang zag niemand het — want de twee instrumenten werden nooit naast elkaar gelegd. Bijvangst die de klasse verklaart: `replay-case.sh` was het enige script in de keten dat rood of groen beslist, en het enige zonder contract-test (gedicht in umanex/umanex-os#269). *Herkenningsteken:* je tikt een controle-commando over in plaats van de oordelende runner aan te roepen. Roep de runner aan — en heeft die geen contract-test, dan is dát je eerste bevinding.
...
*En een zelftest die één eenheid muteert, bewijst niets over de andere.* GEMETEN 2026-09-16 in datzelfde script en zijn kopie in `packages/ui` (umanex-apps, `58b4395`): hoogte, padding, gap en radius (pixels) én opacity (een fractie 0–1) werden met één tolerantie vergeleken, `|a − b| <= 0.51`, en de zelftest muteerde alleen pixelvelden. Elk opacity-verschil tot een halve dekking viel daardoor per constructie binnen de marge, terwijl kop en slotregel opacity als vergeleken veld noemden; een tegenproef in Figma (0,7 → 0,8) bleef groen. Na een eigen tolerantie voor fracties kwam meteen een echt verschil boven: `Slider/disabled=true` op 0,5 in Figma tegen 1 in de browser, omdat `disabled:opacity-50` op een `<span>` stond die nooit `:disabled` matcht — een uitgeschakelde Slider oogde in elke app als een actieve. Recept: geef elke drempel de eenheid van zijn veld, en laat de zelftest **per veldsoort** één geval rood maken (`opacity 0,7 -> 0,8: rood`). *De tegenproef draagt het defect zelf* (2026-08-25) en *verander het object* (2026-08-27) stonden al in `CLAUDE.md` toen beide scripts gebouwd werden (2026-09-07 en 2026-09-16).
...
*Een check die de regel van de code narekent, kan per constructie niet rood worden.* GEMETEN 2026-09-17 (Columba `apps/verkeersanalyse`, `scripts/verwerking-check.mjs`, ronde 2 van de Beoordeel-stap): de check *"over het hele scrollbereik markeert de nav de sectie op de leeslijn"* scrolde in elf stappen, rekende per stap zélf uit welke sectie de leeslijn (48 px) gepasseerd was, en eiste dat `aria-current` daarmee overeenkwam. Dezelfde regel, dezelfde invoer — dus hij bevestigde dat de implementatie doet wat de implementatie doet, en stond groen op **119/119 in twee runs** terwijl het scherm stuk was: de secties onder Context zijn samen korter dan het venster, halen de leeslijn nooit, en de nav markeerde "Context" over het hele bereik. Onafhankelijk gemeten in ronde 3: over 45 monsters in vijf viewports raakten "Manoeuvre" en "Afronding" **0 keer** gemarkeerd, en klikken op drie van de vier nav-items verplaatste de scroll niet. Het onderscheid is scherp te maken: *"de gemarkeerde sectie staat op de leeslijn"* is de **regel**; *"over een volledige scroll komt élke sectie een keer aan de beurt"* en *"een klik zet de sectie echt bovenaan (≤ 4 px)"* zijn **uitkomsten** — die kunnen rood worden, en waren dat ook. Let op de valse reflex: een bevinding die vraagt om "breder bemonsteren" lokt méér meetpunten van dezelfde tautologie uit, en dat blijft een tautologie. Toets dus of je check een andere bron gebruikt dan de code die hij beoordeelt; deelt hij zijn formule met het object, dan meet hij niets.
