...
- **Fix:** Code-guard in `apps/jobradar/scripts/config-scenarios.ts`, aangesloten op `pnpm --filter jobradar scenarios` en daarmee op CI. Hij toetst de *vorm* van de configuratie in plaats van instanties: geen persoonswoorden in `SKILL_KEYWORDS` (met baseline en reden), elk keyword moet zichzelf kunnen vinden, de omschrijving mag de rol nooit veranderen wanneer de titel er een noemt, en de twee assen staan in één `CLUSTERS`-verklaring waaruit `SCORE_SKILLS`/`DESIGN_SKILLS`/`DEV_SKILLS`/`KEYWORD_WEIGHTS` afgeleid worden — plus een vastgepinde verwachtingstabel, zodat een aswijziging op twee plekken moet gebeuren. Tegenproef gedraaid op een kopie in `/private/tmp`: alle drie de historische defecten opnieuw ingevoerd, alle drie gevangen (`'product manager'` → persoonswoord-check; `backend` weer op score 5 → pin-check, óók met een tweede nul-cluster als maskering; onvoorwaardelijk `\b` → `.net` en `c#` vinden zichzelf niet meer). Input opnieuw afgespeeld → `design`, waar de entry `dev` vastlegde. Een vijfde reviewronde legde bloot dat de eerste versie van die guard zélf op een handonderhouden woordenlijst rustte: `'product owner'` passeerde alle 575 checks en verzon drie designclassificaties. Daarom is de eigenlijke bewaking nu **gedragsmatig** (sectie 2b): hij pint wélke keywords in hun eentje, als titel, een classificatie kunnen afdwingen — 52 stuks, allemaal technologieën. Groeit die verzameling, dan groeit beslissingsmacht en faalt de guard. Hij kent geen persoonswoorden en heeft ze niet nodig; getoetst met zeven rolwoorden, waarvan `'ux lead'` uitsluitend door de pin gevangen wordt. **Grens:** de guard dwingt de keuze zichtbaar af, maar kan niet beoordelen óf een nieuw cluster of keyword de juiste as krijgt — dat blijft een oordeel, alleen niet langer een stilzwijgend oordeel.
- **Status:** promoted
