...
2. **Waar landt de spacing-schaal?** In beide themes staat `Layout/Scale` op *source*, dus die levert geen variabelen, terwijl 940 bindingen eraan hangen. Als aparte set exporteren (eigen collectie, één mode — dicht bij het huidige `Base`), of in beide themes op *enabled* (dan zit spacing in de rollen-collectie, met per mode dezelfde waarde)? **Bijgesteld 2026-09-17:** "in beide themes" vervalt — er blijft één theme over. En de dark-mode-briefing zet de layout-rollen in een eigen set `Layout/Roles` naast `Layout/Scale`, dus de vraag is nu of díe twee samen één geometrie-collectie worden (de plek van het huidige `Base`) of twee.
