...
6. **Bij meerdere verbonden bestanden: assert het doelbestand in élke execute.** De actieve file van de Bridge kan stil terugwisselen (reconnects); begin elke `figma_execute` met `if (figma.root.name !== '<doel>') return {fout: …}`. Zonder die guard meet je — of erger, schrijf je — in het verkeerde bestand, en een gelijkende component-set maakt de foute meting onzichtbaar (gemeten: een Luminus-set beantwoordde een Columba-vraag met plausibele getallen).
