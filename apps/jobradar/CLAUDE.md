...
Vacature- en lead-tracker voor UX/UI-freelancers, plus het voorbereidingsplan van umanex.
Next.js 15 (App Router, React 19), dev op poort 3003.
...
| **Bedrijfsplan** | Eigen route `/plan` sinds 2026-09-16: het voorbereidingsplan voor de start van umanex (22 acties, vier beslismomenten). Eigen model (`plan_*` in `lib/db/ddl.ts`) en niet `next_actions` uitgebreid — dat draagt per constructie één actie per bedrijf en kent geen afhankelijkheden, bewijs of prioriteitsgroepen. Drie dingen om te kennen vóór je eraan werkt: **blokkade is afgeleid** (`lib/plan/afleiding.ts`, nooit opgeslagen — een opgeslagen blokkade veroudert stil zodra de rij waaruit hij volgt van status wisselt) · **de seed draait één keer per `SEED_VERSIE`**, met een poort in `settings`; per-rij `ON CONFLICT DO NOTHING` alleen zou een door Jeroen verwijderde afhankelijkheid terugzetten, want die kant bestaat dan niet meer en botst nergens mee · **alles in `lib/plan` is synchroon**, want `db.transaction` van better-sqlite3 commit zodra de callback terugkeert en een async callback commit dus vóór de eerste query. Koppelen aan bedrijven gebeurt vanaf de kaart (`ContactPanel` → *Voorbereidingsplan*), niet met een picker in het plan: zo ontstaat er geen tweede registratie van prospectgegevens. |
...
|---|---|
| **Render vastleggen** | `pnpm --filter jobradar flow --shot=.flow-shots` — legt per route een full-page PNG vast op de verse build. Gemeten: `index.png`, 27 KB. `.flow-shots/` is gitignored: het is bewijs van één run, geen artefact om te bewaren.<br><br>`--smal=400` legt er per route een tweede opname naast op die breedte, en **meet** daarbij `scrollWidth` tegen `clientWidth` — horizontale overloop is het defect dat een smalle opname hoort te vangen, en dat is telbaar. Bewust geen tweede volledige meetronde: de BACKLOG verwierp meten op meerdere viewports (jobradar is een desktop-triagescherm), maar één beeld om naar te kúnnen kijken is iets anders dan een as die rood kan worden. |
| **Flow aandrijven** | `pnpm --filter jobradar flow` — Playwright op een verse build. Laadt elke route (`/`, `/instellingen` en `/plan`), drijft één echte interactie aan (het **status**-filter: `"" → "new"` — de harness pakt `select:visible` en het regio-filter bestaat uit checkboxes), en telt console-fouten. De interactie doet vooraf een expliciete `goto` naar `/`: zonder die regel verhuist hij mee naar de láátste route en valt hij stil terug op een linkklik. `--headed` om mee te kijken. |
...
| **Request/response aandrijven** | `pnpm --filter jobradar opvolging:probe <werkmap>` — bouwt in `.next-apiprobe`, start op **3117** tegen een verse database in de meegegeven map, en rijdt de opvolging-routes af: contactmoment op een lead en op een prospect, de statusovergang, de opt-out-rem (409), een datum in de toekomst (400), een onbekend kanaal (400), een onbekend bedrijf (404), één volgende actie die de vorige vervangt, en het verwijderen van een moment. Raakt `.next` niet en raakt `.data/jobradar.db` niet. Dit was tot 2026-09-09 een gat: er bestond geen instrument dat een route echt aanriep. |
| **Bedrijfsplan visueel** | `pnpm --filter jobradar flow --shot=.flow-shots --smal=400` legt `/plan` vast op beide breedtes, op de database van de tree (bij een nooit gebruikt plan: alles niet gestart). Een **gevúld** scherm legt sinds 2026-09-17 `pnpm --filter jobradar plan:probe <werkmap> --shot=<map>` vast: overzicht, Geblokkeerd opengeklapt, en het paneel na een afronding — op de eindtoestand van de HTTP-gevallen (drie lopend, één wacht op input, beslismoment vastgelegd). |
| **Bedrijfsplan aandrijven** | `pnpm --filter jobradar plan:probe <werkmap>` — bouwt in `.next-planprobe`, start op **3118** tegen een verse database, en rijdt eenendertig gevallen af: de seed over HTTP (22 acties, en een tweede GET zaait niet opnieuw), het versie-conflict (409), een geblokkeerde start met en zonder vastgelegde uitzondering, de vierde actieve actie (409 mét de drie actieve erbij) en het parkeren, afronden zonder én mét bewijs, heropenen met behoud van het bewijs, een cirkel (400), een gemengd verzoek (400), een seed-actie verwijderen (409), een idee opnemen, dubbel koppelen, het beslismoment, de instellingen en de export. Opent met een positieve controle: 22 gezaaide acties en nul geschiedenisregels, anders stopt hij. Raakt `.next` niet en raakt `.data/jobradar.db` niet.<br><br>Geval 31 toetst `vrijgekomen` in het PATCH-antwoord tegen een verwachting die hij uit de rijen zélf afleidt (SQL, niet `uitvoerbaarheidVan`). Daarna draait **`scripts/plan-ui-probe.mjs`** in een browser op dezelfde wegwerp-database — de enige plek waar op Start en Afronden geklikt mag worden. Hij meet de bediening per rij, het inklappen van Geblokkeerd, gewicht via berekende kleuren (geen klassen), de 409 op de focusregel (met loading), een geforceerde 500 en een geforceerde lege groep (beide via een onderschept antwoord, `page.route`), afronden met en zonder gevolg (in beeld, live-regio, orakel uit de ruwe velden), Heropen in hetzelfde paneel, een conflict terwijl het detail nog laadt, focus na Start, de gevolgen bij vervallen, en overloop op 400 px op het gevúlde plan met Geblokkeerd open. Elke as is op 2026-09-17 met een gericht defect rood gezien (drie tegenproef-runs, daarna byte-gelijk hersteld). |
| **Toetsenbordlast op `/plan`** | Uit de flow-harness, op dezelfde database vóór en na de bediening-per-groep (2026-09-17): overzicht **≥ 80 → 43** tab-stops (80 is het plafond van de pass), Acties-tab **76 → 37**, paneel 9 → 9. Het harness-anker voor een actierij is `li[data-actie]`, niet meer `li:has(select)` — de select staat sinds die datum alleen nog in het paneel. |
| **Invarianten op het plan** | Zit in `pnpm --filter jobradar scenarios` als suite `plan` — 320 checks tegen `:memory:`, met tegenproef. Dekt wat een probe niet kan: de seed-inhoud zelf (22 keys, 18 kanten, geen cirkel), idempotentie mét een bewerkte titel en een verwijderde kant ertussen, de afgeleide blokkade in al haar vormen, de focusregel bij een andere limiet, en dat `null` uren nooit als 0 meetelt. |
