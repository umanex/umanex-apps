...
| **Bedrijfsplan visueel** | `pnpm --filter jobradar flow --shot=.flow-shots --smal=400` legt `/plan` vast op beide breedtes. Voor een gevúld scherm is er geen instrument: de toestanden (afgerond met bewijs, drie actieve acties, een beslismoment klaar) zijn op 2026-09-16 eenmalig via de API gezet op een wegwerp-database, met Playwright ernaast. Dat was een meting, geen instrument — wie het opnieuw wil, zet die toestanden opnieuw. |
| **Bedrijfsplan aandrijven** | `pnpm --filter jobradar plan:probe <werkmap>` — bouwt in `.next-planprobe`, start op **3118** tegen een verse database, en rijdt dertig gevallen af: de seed over HTTP (22 acties, en een tweede GET zaait niet opnieuw), het versie-conflict (409), een geblokkeerde start met en zonder vastgelegde uitzondering, de vierde actieve actie (409 mét de drie actieve erbij) en het parkeren, afronden zonder én mét bewijs, heropenen met behoud van het bewijs, een cirkel (400), een gemengd verzoek (400), een seed-actie verwijderen (409), een idee opnemen, dubbel koppelen, het beslismoment, de instellingen en de export. Opent met een positieve controle: 22 gezaaide acties en nul geschiedenisregels, anders stopt hij. Raakt `.next` niet en raakt `.data/jobradar.db` niet. |
