...
```
  ...
  Twee keer op 2026-09-08 ging dit mis, van twee kanten. Eén sessie liet de controles wél draaien maar in dezelfde Bash-aanroep als de `checkout -b`: ze waarschuwden pas toen de branch er al stond, en een onvastgelegde `main.ts` reisde mee. De andere las de passage in zijn vorige vorm — *"kijk naar drie dingen"* — en sloeg hem over; er stonden nul `|| exit`-vormen in deze passage tegen één in het merge-blok. Een controle die niet vóór de actie kan stoppen is een logregel, geen controle. Die derde is de plek die de eerste twee níet tonen: een stash is werk dat niet in `status`, niet in `worktree list` en niet in `log` verschijnt, en dus onzichtbaar blijft in elke controle die daarop leunt. Gemeten op 2026-08-27: een routinecontrole vóór een taak in Luminus vond 1 425 regels ongetrackte briefings en audits, maar zag de stash uit augustus niet — die kwam pas boven toen de gebruiker ernaar vroeg. Beide stashes bleken achteraf redundant, maar dat was toeval, niet het resultaat van de controle. Let bij een oude stash op de vorm van het defect: hij hangt aan de commit waarop hij gemaakt is, dus hoe ouder hij is hoe minder zijn inhoud over vandaag zegt — de umanex-apps-stash van april droeg hógere dependency-majors dan wat productie sindsdien draait (`zustand ^5` tegen `^4`, `date-fns ^4` tegen `^3`), zodat toepassen geen herstel maar een blinde upgrade van een draaiende app zou zijn. En een stash die blijft staan terwijl zijn inhoud óók in de tree zit, is de handtekening van `git stash apply` in plaats van `pop`. In al die gevallen begin je er niet stil een tweede naast, en maak je óók niet stil een eigen tree: meld het en laat Jeroen kiezen — om beurten, eerst afronden, of stashen. Is de tree vrij, vertak dan altijd vanaf `origin/main` en nooit vanaf de HEAD die je aantreft: `git fetch -q origin && git checkout -b <type>/<beschrijving> origin/main`. Een branch vanaf andermans HEAD erft diens commits — dat is het mechanisme van 2026-08-07, en beide hooks zwijgen erover (gemeten).
