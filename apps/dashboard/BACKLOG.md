...
## 2026-09-15 — `next build` herschrijft twee getrackte bestanden naar de distDir waarmee hij draait · [infra]
- **Wat:** Een build met `NEXT_DIST_DIR=.next-harness` zet `next-env.d.ts` op `/// <reference path="./.next-harness/types/routes.d.ts" />` en voegt `.next-harness/types/**/*.ts` toe aan de `include` van `tsconfig.json`. Beide zijn getrackt, en `.next-harness/` is gitignored — dus een commit daarvan laat `type-check` naar een map wijzen die in CI niet bestaat. Gemeten 2026-09-15 bij het bouwen van de flow-harness; de wijziging stond ongevraagd in `git status`.
- **Waarom dit een derde instantie is:** dezelfde klasse als `fix(dashboard): give next dev its own dist dir and port` (2026-09-09, gesloten als `gebouwd` hierboven) en `fix(dashboard): type-check the route types that dev writes`. Die twee losten op dat dev en productie dezelfde `.next` deelden; wat bleef staan is dat élke build zijn distDir in twee getrackte bestanden schrijft. Twee fixes in negentig dagen op dezelfde oorzaak, en niets dat de derde tegenhoudt.
- **Waarom niet nu:** de harness lost het lokaal op — hij maakt een back-up vóór de build en toetst het herstel (`zegt('de build liet next-env.d.ts en tsconfig.json ongewijzigd achter', …)`). Dat is een patch op één plek: wie handmatig `NEXT_DIST_DIR=… next build` draait heeft hem niet, en de volgende app met een harness herhaalt hem.
- **Eerste zet:** één guard in plaats van drie back-ups. Kandidaat: een `pre-commit`-signaal dat een gestagede `next-env.d.ts` of `tsconfig.json` weigert wanneer die naar een gitignorede distDir verwijst. Tweezijdig te toetsen — een verwijzing naar `.next` moet passeren, een naar `.next-dev`/`.next-harness` niet.
- **Check:** `NEXT_DIST_DIR=.next-harness npx next build >/dev/null 2>&1 && git -C . status --porcelain apps/dashboard/next-env.d.ts apps/dashboard/tsconfig.json | grep -c .` vanuit `apps/dashboard` — geeft dat 2, dan herschrijft de build de bestanden nog steeds en leeft dit item. Draai `git checkout --` op beide erna.
- **Status:** open
