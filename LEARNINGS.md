...
## 2026-09-16 — Een getal op het scherm en de verzameling erachter komen uit twee afleidingen
- **Input:** Een klasse uit terugkijken, geen prompt. Zichtbaar met `git log --since="90 days ago" --oneline -- apps/jobradar`, waarin vier commits dezelfde vorm rechtzetten: `49072dc` (de kaart telt op bedrijfssleutel, de doorklik zoekt op titel — 3 tegen 6), `4c6860b` (het plafond wordt per deelvraag bewaakt en tegen de som getest — 480 op het scherm, 40 in de sync), `c0e407d` (de lijst filtert terwijl `/api/kaart` geen enkele parameter las — 217 punten bij elk filter) en `76a29ff` (de ideeën-badge telt alleen de open, de twee badges ernaast het totaal).
- **Fout:** Een aggregaat op het scherm en de lijst of doorklik eronder worden elk apart afgeleid, en die twee lopen uiteen zonder dat iets afgaat. Het bedrog zit in de vorm: het getal ís waar voor zijn eigen afleiding, dus niets voelt fout tot iemand de twee naast elkaar legt. `c0e407d` wijst de oorzaak zelf aan — *"The fix is the duplication, not the missing parameter"* — maar alleen voor dat ene paar; de andere drie zijn los gepatcht en geen van de vier commits noemt een ander.
- **Ongemeten as:** er is per paar een eigen check (`signal-scenarios.ts` voor kaartwaarde ↔ bedrijfssleutel, `adzuna-scenarios.ts` voor het plafond, `kbo-scenarios.ts` plus de flow-harness voor lijst ↔ kaart) en niets dat de regel zelf toetst: *elk aggregaat op het scherm en de verzameling eronder komen uit één declaratie.* Daar bovenop draait de enige as die deze klasse kán vangen — een tweede filterstand meten in plaats van tellen tégen het antwoord, `flow-harness.mjs` — in geen enkele CI-stap: van jobradar draait `ci.yml` uitsluitend `pnpm --filter jobradar scenarios`. Dat gat staat al als los item in `BACKLOG.md` sinds 2026-08-27.
- **Status:** open
