...
- [ ] **`fontSize.114` + hero-token herbestemmen** — beslist Albert Sans Bold (14-jul). **Tokens Studio (2 edits):** (1) hernoem het `fontSize`-token `124` → `114` en zet z'n waarde `96` → `114` (ruimt de key≠waarde-landmijn op; enige consumer is de hero); (2) in `type.heroNumeric`: `fontFamily` → `{fontFamily.albertSans}`, `fontSize` → `{fontSize.114}` (fontWeight.bold / lineHeight.tight / letterSpacing.displayTight staan al goed). **Daarna ik:** `activeStyles.heroText` → `...typeStyles.heroNumeric` (hardcode 114 + `// TODO` weg; -4.5% × 114 = -5.13 reproduceert exact). `[code-TODO]`
...
- [ ] **Re-triage `briefings/2026-07-04-audit-design-vs-code.md`** — 214-item beslisdocument, 0 afgevinkt, maar **twee headline-claims zijn intussen bevestigd-stale** (14-jul): Cluster 1 (token-pipeline `[object Object]` in `constants/colors.ts`) is opgelost, en Cluster 2 (cyaan in Profile-sheets + toast) staat niet in de code — beide al rood/correct. Het hele doc verdient een herweging i.p.v. blind afwerken; verifieer elke cluster tegen de huidige code vóór actie. `[audit 2026-07-04]`
