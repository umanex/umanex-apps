...
- **Klant:** umanex
- **Status:** gevalideerd — 2026-09-19, alle 58 acceptatie-items op bewijs, 0 bevindingen in de flow-run
...
- [x] Interactie — klik en toetsenbord, gedekt door de `aria-current`- en focus-items hieronder — bewijs: gedekt door c02f (klik, 3 gevallen) en de tab-volgorde-as (3 routes), beide in flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Edge cases — querystring, hash, foutpagina, 404 en 400 px, elk een eigen item hieronder — bewijs: vijf eigen items hieronder, elk groen in flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)

### De balk staat er en weet waar je bent

- [x] 3 navigatielinks in `header` op `/` — noemer: alle `header a`-elementen — bewijs: `/ navigatie: 1 van 3 links met aria-current`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] 3 navigatielinks in `header` op `/plan` — zelfde noemer — bewijs: `/plan navigatie: 1 van 3 links`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] 3 navigatielinks in `header` op `/instellingen` — zelfde noemer — bewijs: `/instellingen navigatie: 1 van 3 links`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] `[aria-current="page"]` = 1 op `/plan`, mét noemer 3 — bewijs: `1 van 3`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] `[aria-current="page"]` = 1 op `/instellingen`, mét noemer 3 — bewijs: `1 van 3`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Op `/` wijst de `aria-current`-link naar `/` — gelezen uit `href`, niet uit de linktekst — bewijs: `aria-current (/)` uit het `href`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Op `/plan` wijst de `aria-current`-link naar `/plan` — gelezen uit `href` — bewijs: `aria-current (/plan)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Op `/instellingen` wijst de `aria-current`-link naar `/instellingen` — gelezen uit `href` — bewijs: `aria-current (/instellingen)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] `/?tab=leads&status=alle` geeft `aria-current` op de link naar `/` — een querystring zet de markering niet uit — bewijs: `navigatie (querystring): 1 van 3 links met aria-current (/)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] `/instellingen#bedrijfsplan` geeft `aria-current` op de link naar `/instellingen` — een hash zet de markering niet uit — bewijs: `navigatie (hash): 1 van 3 links met aria-current (/instellingen)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] De geforceerde foutpagina draagt 3 navigatielinks in `header` (tweede server, `JOBRADAR_DB_PATH` naar een tekstbestand) — bewijs: `c02e: foutpagina navigatie: 1 van 3 links met aria-current (/)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] Het wordmerk is geen kop — `header h1,header h2,header h3,header h4,header h5,header h6` = 0 op alle drie de routes — bewijs: `0 koppen in de balk` op alle drie de routes én op de foutpagina, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)

### De oude headers zijn weg

- [x] 0 links met de toegankelijke naam "Terug naar het dashboard" op `/plan` — bewijs: tab-volgorde-note `/plan`: `a(Radar) → a(Plan) → a(Instellingen) → a(Exporteer (Markdow) → a(JSON) → …`, geen terug-link, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] 0 links met de toegankelijke naam "Terug naar het dashboard" op `/instellingen` — bewijs: tab-volgorde-note `/instellingen`: `a(Radar) → a(Plan) → a(Instellingen) → button(UX verwijderen) → …`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] De foutpagina houdt die link wél — precies 1, en een klik erop is een documentnavigatie (c02b) — bewijs: `c02b` groen op alle drie de routes: 1 eigen uitweg, documentnavigatie naar `/`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] Precies 1 h1 op `/` — kopstructuur-pass van de harness — bewijs: `/ kopstructuur: 28 koppen, niveaus h1 → h2 → h3`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Precies 1 h1 op `/plan` — kopstructuur-pass — bewijs: `/plan kopstructuur: 12 koppen, niveaus h1 → h2 → h3`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Precies 1 h1 op `/instellingen` — kopstructuur-pass — bewijs: `/instellingen kopstructuur: 3 koppen, niveaus h1 → h2`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] Op `/plan` is die h1 wél zichtbaar — breedte > 1 — bewijs: `/plan: h1 zichtbaar (862px breed)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Op `/instellingen` is die h1 wél zichtbaar — breedte > 1 — bewijs: `/instellingen: h1 zichtbaar (704px breed)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] De twee export-links van het plan staan er nog — "Exporteer (Markdown)" en "JSON" elk 1 keer op `/plan` — bewijs: tab-volgorde-note `/plan`: `a(Exporteer (Markdow) → a(JSON)`, elk één keer, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] Sectie Bedrijfsplan: precies 1 knop met de primaire achtergrondkleur — zelfde meting, eigen noemer — bewijs: `c51: 1 van 1 actieknoppen in Bedrijfsplan is gevuld ("Opslaan")`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] Focus-pass groen op `/` — 0 stops zonder zichtbare focus, mét noemer — bewijs: `/ toetsenbord: 80 stops, elk met zichtbare focus` (80 = plafond), flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Focus-pass groen op `/plan` — 0 stops zonder zichtbare focus, mét noemer — bewijs: `/plan toetsenbord: 44 stops, elk met zichtbare focus` (was 43 vóór de balk), flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Focus-pass groen op `/instellingen` — 0 stops zonder zichtbare focus, mét noemer — bewijs: `/instellingen toetsenbord: 24 stops, elk met zichtbare focus`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] De drie navigatielinks zijn de eerste drie tab-stops op `/plan` — zelfde note — bewijs: zelfde drie stops, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] De drie navigatielinks zijn de eerste drie tab-stops op `/instellingen` — zelfde note — bewijs: zelfde drie stops, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] Namen-pass: 0 bedienbare elementen zonder berekende naam op de drie routes, mét noemer — bewijs: `/` 115, `/plan` 45, `/instellingen` 27 bedienbare elementen, elk met een naam, 0 uitzonderingen, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] `/plan` op 400 px: `scrollWidth ≤ clientWidth` — de enige route die als smal doelwit geldt — bewijs: `/plan op 400px: geen horizontale overloop (400 ≤ 400)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] `/instellingen` op 400 px: `scrollWidth ≤ clientWidth` — bewijs: `/instellingen op 400px: geen horizontale overloop (400 ≤ 400)`, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
...
- [x] `main [data-sync-knop]` = 1 op `/` — bewijs: zelfde meting, beide kanten in één regel, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
- [x] De doorklik "+N vacatures" zet nog steeds tabblad en vinkje — de bestaande sectie `flow --alleen=fase3-nieuw` groen, mét het aantal checks — bewijs: sectie `fase3-nieuw` groen in flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0); c13d/c13e onderscheppen de sync en toetsen tabblad + vinkje

### De instrumenten blijven meten

- [x] `flow-harness.mjs` klikt de link "Plan"; de count-guard eist er precies 1. Tegenproef: de naam terugzetten op "Bedrijfsplan" → rood — bewijs: `triage: na /plan en Back staan filter én URL nog op de gefilterde stand`, met de nieuwe telling-guard ervoor, flow-run 2026-09-19 (240 checks, 0 bevindingen, rc=0)
