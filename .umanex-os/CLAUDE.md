...
*Een grep-treffer is niet de entry.* De `Status` van een lus-item staat buiten het venster van een kale `grep -n` — over 79 entries in de drie lus-bestanden gemiddeld +4,4 tot +4,9 regels lager, in de staart +11. Een groter venster is een nieuwe proxy; anker op de entry:
...
*Bij afhankelijke berekeningen is de invariant de meetbare as.* Volgt een waarde uit een andere, dan valideert scherm-per-scherm niets: elke fix is lokaal correct terwijl dezelfde afgeleide waarde elders anders berekend blijft. PLAN levert minstens één **invariant** over het hele model, en BEOORDEEL rékent die uit over een echte dataset in plaats van de uitkomst af te lezen.
