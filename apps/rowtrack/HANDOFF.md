...
- **Check:** `node -e 'const m=require("./figma/build-spec.min.json"); for(const [n,d] of Object.entries(m.componenten)) if((d.slots??[]).some(s=>/_[a-z]{2,}$/.test(s))) console.log(n, d.slots.join(","))'` in `apps/rowtrack` — elke regel is een component met minstens één machinale naam.
