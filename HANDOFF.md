...
## 2026-09-14 — Een SHA-verankerde HANDOFF-check leest in een remote sessie stil verkeerd · [risico]
- **Bevinding:** Veel HANDOFF-checks hebben de vorm `git log --oneline <sha>.. -- <pad>`, met de conventie *leeg = ongewijzigd*. Een Claude Code-sessie in de cloud krijgt een **shallow clone** (gemeten vandaag: `is-shallow-repository` = `true`, 414 commits, geënt op 2026-08-08), dus elke SHA van vóór die ent bestaat er niet. Het commando geeft dan `fatal: bad revision` op **stderr** en een **lege stdout** — precies het signaal dat de conventie als "ongewijzigd" leest. Tweezijdig gemeten: gebroken check `stdout=[] rc=128`, werkende check `stdout=[1 regel] rc=0`. Drie rowtrack-items (`best_2k_seconds`, de Storybook-rails, BottomSheet) lazen zo eerst als "niets veranderd"; ná `git fetch --unshallow` (414 → 1820 commits) vuurden alle drie wél. Dit is dezelfde klasse als *een lege uitkomst vraagt een positieve controle* uit `.umanex-os/CLAUDE.md`, maar de checks zelf dragen die controle niet.
- **Check:** `git rev-parse --is-shallow-repository` — `true` betekent dat elke SHA-verankerde check in deze sessie stil verkeerd kan lezen en eerst `git fetch --unshallow` nodig heeft. En `grep -c 'git log --oneline [0-9a-f]\{7\}\.\.' HANDOFF.md apps/*/HANDOFF.md` telt hoeveel checks deze vorm hebben.
- **Volgende zet:** Kandidaat voor `vastleggen` — dit is een defect van de check-conventie, niet van een app, en het propageert naar élke repo met HANDOFF-items. Twee kandidaat-remedies, geen van beide getoetst: de checks laten beginnen met een bestaans-assert (`git cat-file -e <sha>^{commit} || echo "ONMEETBAAR — shallow"`), of `session-start-handoff.sh` laten unshallowen wanneer de clone shallow is. Jeroen kiest; ik heb de conventie niet eenzijdig aangepast.
- **Status:** open
