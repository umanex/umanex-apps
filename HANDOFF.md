...
- **Tweede keer, zelfde dag (later):** na een bouw met height-bindingen (kinderen `141:67`/`141:70`/`141:75`, read-back `size-control-md` op beide footer-knoppen) stond DialogContent een half uur later terug op de kinderen `141:53`/`141:56`/`141:61` van de bouw daarvóór, zonder height-binding, `bouwbezig` leeg. Exact oude node-ids terug is het patroon van een undo in Figma Desktop; een andere sessie die bouwt maakt nieuwe ids. Alleen die ene bouw was geraakt: Base (57), de 11 rollen en de 213 herbindingen stonden er nog. De sessie schreef daarna niet meer naar Figma.
- **Oorzaak gevonden (2026-09-17, avond):** er werkt een tweede sessie in hetzelfde Figma-bestand. Gemeten na Jeroens publish: Badge draagt live een `size`-as met 12 varianten tegen 6 in het manifest op `origin/main` — het werk van `feature/ui-badge-slider` (fase 4a), gebouwd terwijl deze sessie Dialog bouwde. Beide voorvallen (een tweede set kinderen erbij, en een set die verdween) passen op undo/redo in dat gedeelde bestand. Derde keer opgeruimd na een versie-checkpoint; Dialog staat op 3 kinderen, publicatie CURRENT.
- **Regel hieruit:** vóór een Figma-bouw vragen wie er in het bestand zit, en ná elke bouw de kinderen tellen — `parity` en `toets-batch` zagen de dubbele set niet, het beeld wel.
...
- **Check:** `grep -A4 "NIET_VISUEEL = {\|FIGMA_ONLY = {" packages/ui/scripts/figma/doel.mjs` — de lijst is kort en leesbaar; klopt elke regel nog met wat je van het component verwacht?
...
- **Doorgelopen (2026-09-16, batch 0 van de shadcn-bibliotheek):** de packages/ui-lijst staat nu in `packages/ui/scripts/figma/doel.mjs`, gedeeld door `figma-sync-check.mjs` én de bouwspec — voorheen had alleen de guard hem, en een tweede lezer met een eigen kopie zou stil uiteenlopen. Nagekeken tegen de code: `Input.type` verandert het veld niet (één `className`, geen tak op `type`), `Tooltip.side` bepaalt alleen de plaatsing t.o.v. de trigger, `ThemeToggle.mode` is nog steeds `useState(isDark)` zonder prop — alle drie nog juist. Switch en Dialog voegden geen uitsluiting toe. De vraag aan Jeroen blijft dezelfde; batch 2 (Popover, HoverCard: `side`) zal er twee bij zetten.
- **Status:** open
