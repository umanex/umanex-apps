...
Voor elk Figma-gerelateerd werk: **lees de node-id uit een gegenereerd artefact**, niet uit een
bijgehouden tabel. Drie bronnen, elk met één taak:

| wat je zoekt | bron |
|---|---|
| een library-pagina en zijn primary node | `figma/manifest.json` → `pages[<naam>].primary.id` |
| de key en de slots van een component (voor een instance) | `figma/library-component-keys.json` |
| welke schermframes er zijn en hoe ze heten | `scripts/schermen.mjs` |

`figma-map.md` was die tabel en is het niet meer: hij is voor het laatst met de hand gesynct op
2026-07-15, wijst naar node-ids op de oude *Screens*-pagina, noemt paden die niet meer bestaan, en
wordt door geen enkel script gelezen. Hij staat er nog als historiek. Gokken op een componentnaam
blijft fout — maar de manifest is de plek die het antwoord kent.
...
| Schermpagina's | **geen** — de schermen staan sinds 2026-09-08 niet meer in dit bestand maar als frames op *Screens v2* in `T1bGrvIzSNeLyh5CbarATZ` (24 bij de export van 2026-09-09, 28 sinds 2026-09-14), opgebouwd uit instances van deze library |
...
| **Figma ↔ code toetsen** | `pnpm --filter rowtrack figma:check` — zestien assen (dekking, pagina's, variant-assen, variant-nodes, tokennamen, tokenwaarden, typografie, deep-links, hardcoded waarden, aantal ongebonden waarden, publicatievenster, herkomst van de laagnamen, instancevulling, eigenschappen — elke tekst-property heeft een node en geen stam komt dubbel voor, **vertaalrest** — elke marge die Figma niet kan uitdrukken staat op een tweezijdige ratel en wordt bij naam genoemd, **namen** — elke machinale property-naam is geaccepteerd of rood). De laatste twee zijn de consumptiepoort van `verify` rail 15: een meldingenlijst zonder ratel is stilte met een teller. Vereist een verse `figma/manifest.json`; zie *Figma-manifest verversen* hieronder. |
...
| **Routes en tabbar (invariant)** | `pnpm --filter rowtrack routes` — toetst dat de routes die expo-router in `app/(tabs)/` aanmaakt exact de namen zijn in `<Tabs.Screen name=…>`, in béide richtingen, en dat `app/` geen `.stories.*` draagt. Bestaat sinds 2026-09-14: `app/(tabs)/profile.stories.tsx` stond als **vijfde tab** in de uitgeleverde app en crashte hem bij een tik (PR #478). Drie expo-router-regels zitten erin die je niet kunt raden: alléén `_layout` is een layout (elk ánder `_bestand` is gewoon een route), een map zónder `_layout` is geen navigator maar laat zijn routes híjsen, en een map zonder routebestanden levert niets op. Leest alleen `app/`, draait in milliseconden, en staat daarom vooraan in de CI-sectie — achter de Figma-guards zou een rode parity hem nooit laten draaien. |
| **Routes-tegenproef** | `pnpm --filter rowtrack routes:selftest` — elf gevallen op een wegwerpkopie van de echte boom, mét drie negatieve controles: een lege map hoort GROEN te blijven, een parser die geen enkele `Tabs.Screen` matcht hoort te stoppen, en een die er **één** niet leest ook (drie van de vier gelezen ziet er gezond uit en is het niet). Elke mutatie eist een uitkomst op zijn effect: een mutatie die niets raakte telt als gezakt, niet als geslaagd. Stand 2026-09-14: 11/11. |
...
| **Figma ↔ browser (beeld)** | `pnpm --filter rowtrack beeld` — legt de gecommitte Figma-export (`figma/beelden/`, 28 frames, ~1,1 MB) naast een verse browser-render van dezelfde story, en diff't in Chromium's canvas (geen dependency). Dit is de as die `parity` per constructie NIET heeft: kleurwaarde, icoonvorm, `text-transform` en **breedte** — dat laatste staat op élke node buiten parity omdat twee tekstengines dezelfde tekst anders meten, en juist daar leefde de drift. Gemeten 2026-09-09: elke formulier-instance stond 390 breed met inhoud van 224, met dertien guard-assen groen. Maskeert de Ionicons-glyphs (die bestaan niet in Figma) identiek op beide beelden. **De vloer is gemeten**: `ResetPasswordScreen`, het enige scherm zonder instances, wijkt 0,02% af — dat is de renderer-ruis. Zonder `--drempel` velt het script geen oordeel — maar hij is niet lees-only: hij herschrijft élke run `figma/beeld-verschillen.json`, en dat bestand staat in git. Gemeten 2026-09-16: een run op ongewijzigde code gaf 46 gewijzigde regels, allemaal de datum plus de gerandomiseerde confetti van `ActivePhase/Doel-Bereikt`. Reken er dus op dat `beeld` je tree vuil achterlaat; commit het rapport wanneer de meting telt, zet het terug wanneer je alleen keek. Een tolerantie kies je pas als de echte verschillen weg zijn. **En sinds 2026-09-10 is het aggregaat ontleed:** per frame splitst hij `grof` in `tekst` (binnen een tekstgebied van de browser-render — Figma's tekstengine meet dezelfde tekst anders, daar kan de bouw niets aan doen) en `overig` (wat een klasse verdient). Gemeten over 24 frames: 56,23 = 42,55 tekst + 13,74 overig, en tien frames onder 0,1 % overig. De drempel hoort dus op `overig`; een som van `grof` is een richting zonder vloer, en de acceptatie-guard vuurt sinds die dag op zo'n som zonder ontleding. `--schrijf` legt per frame een 3-luik in `figma/beeld-diff/` (gitignored). |
