...
- **Update 2026-09-18:** `Button` heeft nu `size="xs"` (28px) en `icon-xs` (28 × 28), gebouwd voor de ledger van cashflow. Het 28px-cluster van jobradar past daarmee zonder maatwijziging; het 24px-cluster wordt 4px groter in plaats van 12. De vraag is daarmee versmald: nog één keer meten hoeveel van de 21 knoppen in welk cluster vallen, en of de 24px-groep die 4px mag hebben — niet meer of er een maat bij moet.
- **Status:** open
...
- **Update 2026-09-18:** gebouwd als `size="xs"` (en `icon-xs`) op `Button` en `Input`, op een nieuwe rol `size.control-xs` = `{spacing.7}` — dus in de primitive, niet in een lokale wrapper (keuze Jeroen). De maat heet `xs` en niet `compact`, zodat hij in de reeks `xs · sm · lg` valt. 28px gemeten tegen wat de ledger vandaag tekent; `figma:check` stond rood op de maat-as en staat na de Figma-stap groen op 32 checks. Briefing: `briefings/2026-09-18-component-xs-maat.tcebc.md`. De consumenten omzetten is stap 6 van het cashflow-plan en blijft open.
- **Status:** gebouwd
