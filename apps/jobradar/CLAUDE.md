...
| **Invariant draaien** | `pnpm --filter jobradar scenarios` — 352 invarianten over drie suites: de scorekern en signaal-afleiding (`signal-scenarios.ts`), de sync-upserts tegen een `:memory:`-database met het echte schema (`upsert-scenarios.ts`), en de Adzuna-ophaallaag met een gestubde `fetch` (`adzuna-scenarios.ts`). Elke suite draait zijn tegenproef ervóór — `SCENARIO_SELFTEST=1` injecteert één check die móét falen, en die run hoort niet-nul te eindigen. Geen netwerk, geen database op schijf, geen transpiler: Node 24 stript de types zelf en `scripts/ts-resolve.mjs` lost de extensieloze relatieve imports op. |
