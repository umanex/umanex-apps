...
- **Status:** grotendeels resolved — 2026-07-11: verifieerd op een echte 657m/310s rit (workout `ec6844d3`, 311 samples). `best_2k_seconds`=null (te-korte-sessie edge case → tile "—", geen valse PR); capture exact ~1Hz; first `[0,0]`/last `[310,657]`, monotoon, max gap 1s (pairing-fix hield, geen `t=0`-collapse, geen dropouts); `bestTimeForDistance` matcht brute-force op de échte data voor 500/250/100m (best-500m 3:33). Deze erg telt tijd dóór tijdens rust (43s rust in de reeks) → conservatieve pauze-afhandeling bevestigd. **Rest open:** één échte ≥2000m rit die een concreet getal op de tile rendert (happy-path number), en payloadgrootte op een lange rit (657m ≈ 311 tuples, dus ~3600 voor 2k = ruim binnen jsonb).
