...
- [ ] **`fontSize.114` → verplaatst naar sectie 3** — hangt vast aan de hero-font-beslissing (Albert Sans Bold vs Source Serif) én de `fontSize.124 = "96"`-landmijn (key ≠ waarde, gebruikt door `type.heroNumeric`). Los toevoegen kan, maar correct wiren hoort bij die beslissing. `[code-TODO]`
