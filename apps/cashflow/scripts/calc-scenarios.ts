/**
 * De gedeelde generator en digest achter `calc-baseline.ts` en `calc-regression.ts`.
 *
 * Bouwt 300 deterministische scenario's, rekent ze door met `computeAnchorState` en
 * `calculateMonths`, en vat elke maand samen in een digest van élke berekende waarde —
 * inclusief de formules die de UI vandaag zelf uitrekent, als bevroren kopie.
 *
 * Twee lezers, twee vragen:
 *   - `calc-baseline.ts` dumpt de digest als JSON om vóór en ná een refactor te diffen.
 *   - `calc-regression.ts` hasht diezelfde JSON en vergelijkt met een vastgelegde hash, zodat
 *     een onbedoelde verschuiving in CI rood wordt in plaats van alleen in een handmatige diff.
 *
 * Gemeten bij de extractie (2026-09-16): de dump is byte-identiek aan die van vóór de splitsing
 * en onafhankelijk van de tijdzone (Europe/Brussels, UTC en America/Los_Angeles geven dezelfde
 * sha256) — de rekenkern leest de datum van vandaag nergens.
 */
