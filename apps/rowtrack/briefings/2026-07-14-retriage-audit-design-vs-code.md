...
1. **Profile-sheet polish-pass** — **code-pass gedaan (14-jul)**: 6 token-valide alignments doorgevoerd — sheet-padding 20→32, input-radius 16→12 (`componentRadius.cardSm`, 2×), rij-volgorde LICHAAMSGEGEVENS (Geboortedatum → pos 2), lijstrij-divider full-width, huidig-email 16→18px, streefwaarde-unit 16→12px. tsc groen. **Bewust NIET (code correct / geen off-token hardcodes):** off-token kleuren `#1A1F2E`/`#94A3B8`/`#47556E`/wit → Figma tokeniseren; off-scale maten (stepper 52px, segment-radius 10) → nieuw token nodig; chevron blijft Ionicons (projectconventie); copy-labels + datumformaat `11 jul 1989` → code-voice behouden; GoalProgressCard-borders → shared met Home (daar niet geverifieerd), overgeslagen; code-only states/labels → werkstroom 4.
