...
*Een instrument dat draait is nog geen instrument dat meet.* Vóór je een uitkomst vertrouwt, toon je dat hij hád kunnen afwijken. Vier richtingen waarin dat misgaat:

- **Een guard** toets je op **béide** kanten: één geval waarin hij moet zwijgen, één waarin hij moet afgaan. Zonder de zwijg-kant weet je niet of hij vals alarm slaat; zonder de afgaan-kant niet of hij nog iets meet. Een wachter die permanent afgaat leert je hem te negeren — schadelijker dan geen wachter.
- **Een lege uitkomst** vraagt een positieve controle: "niet gevonden" en "instrument kapot" zien er identiek uit. Laat hetzelfde instrument in dezelfde run iets vinden waarvan je wéét dat het bestaat; vindt het dat niet, dan is de meting ongeldig, geen resultaat. Spreekt een onafhankelijk feit de lege uitkomst tegen (een binding die uit een library komt terwijl de library-lijst leeg oogt), dan is die tegenspraak het alarm — verklaar hem vóór je iets concludeert.
- **Een groene uitkomst** vraagt een negatieve controle, want een gevulde, wáre uitkomst kan over de verkeerde grootheid gaan. "De reaction bestaat, de trigger is `ON_CLICK`, de bestemming is geldig" is vier keer waar en nul keer een antwoord op *gebeurt er iets als je klikt*. Benoem het defect dat de check moet vangen en laat hem erop afgaan — op de toestand vóór de fix, op een kapot exemplaar elders, of op een geconstrueerd geval. Blijft hij groen mét het defect, dan is het een vorm-check die zich voordoet als gedragscheck. Dezelfde toets ontmaskert de tautologie: ligt de uitkomst per constructie vast (`som == hoogte` waar die hoogte uit de som volgt), dan meet hij niets, hoe precies hij ook rapporteert.
- **Een vervangen instrument** — een herschreven dump-filter, vergelijker of parser — zegt pas iets over het gewijzigde deel als het het óngewijzigde deel exact reproduceert. Gemeten op fleet-manager: een nieuw dump-filter gooide twee strings weg en die lege uitkomst zag er identiek uit aan een schone dump; alleen 19 onveranderde schermen ernaast leggen (aantal teksten + tekenlengte) haalde het boven. Dat het instrument zélf kapot kan zijn hoort bij die toets: de vergelijker gaf twee verschillende hashes voor identieke invoer.

Kun je het gedrag niet opwekken, dan is het antwoord `[NIET TE VERIFIËREN — reden]`, nooit een goedkopere proxy: de moeilijk meetbare as krijgt dezelfde lat als de makkelijke, of een expliciet gat. Het scherpste signaal is één taak met twee oppervlakken waarvan er maar één op gedrag getoetst wordt.

*Bij een telling of exit-status bevat het meetbereik vaak de meting zélf.* Een `grep -c` op een bestand dat zijn eigen format-voorbeeld draagt telt dat voorbeeld mee; een CI-log bevat het script dat hij logt, dus een grep op een `echo`-tekst vindt de broncode terug; `$?` na een pipe geeft de status van de láátste pijpcomponent en niet die van je script. Gemeten op één sessie: vier keer, en drie keer met een getal dat plausibel oogde — "3 open entries" klopte precies met de drie die net bijgewerkt waren, terwijl het echte antwoord 0 was. Anker een telling daarom op het **object** (de entry-header, niet de losse string), filter de bron uit de log, en lees een exit-status vóór de pipe (`out=$(cmd); rc=$?`). Een uitkomst die klopt met je verwachting is geen bevestiging zolang je de grootheid niet expliciet hebt afgebakend.
...
*Een naam is een bewering over het ding, niet het ding.* Een laagnaam, een label, een kolomtitel, een regel in een briefing — allemaal beschrijvingen die iemand ooit typte en die sindsdien niet meegroeiden. Identificeer waar je mee werkt daarom aan zijn **inhoud**: de titel op de kaart, de velden in het formulier, de rij in de tabel. Gemeten op LQB: één frame droeg de naam `unit:04-contact`, zijn kind `screen:d1-account-manager-handoff`, terwijl de kaart erin "Add your company details" heet met de velden `Company name` en `Street` — alleen de inhoud zei wat het scherm ís. Spreken twee onafhankelijke signalen elkaar tegen (een diagram-label tegenover een laagnaam, een `@figma`-header tegenover je aanname), dan is die tegenspraak het **alarm**, geen materiaal om stilzwijgend uit te kiezen: verklaar hem, of meld beide. En trek een onbevestigde gevolgtrekking nooit door naar zusters "voor de consistentie" — dan vermenigvuldig je één fout tot vier. Wijkt het geval dat de gebruiker zélf aanwees af van je conclusie, dan is dát de tegenspraak die je verklaart, niet een afwijking die je gladstrijkt.

Twee varianten van dezelfde val. **De render-eigenschap:** een `strokeBottomWeight`, een `visible`, een `gap` is invoer voor de render, niet de render zelf — gemeten op fleet-manager: `strokeBottomWeight: 2` zonder één stroke-paint las als "er staat een onderlijn" terwijl er niets getekend werd, en een tekstnode met `visible: true` onder een ouder op `opacity: 0` las als "hij rendert". **De selector met een broer:** `querySelector('aside')` faalt niet bij twee `<aside>`'s — hij geeft de eerste, en dat is zelden degene die je bedoelt. Gemeten op Columba (2026-08-26): `Sidebar.tsx` en `SidePanel.tsx` zijn allebei een `<aside>`, dus "paneelbreedte 304px" was de sidebar — óók vóór er een paneel bestond, waardoor de toggle 372 ↔ 880 als "verandert niet" las; `querySelector('svg')` pakte een status-icoon en meldde "de glyph hertekent niet", terwijl het juiste anker (`aria-label^="Manoeuvre-plaatje voor"`) 494 → 659 tekens geeft. Anker daarom op wat het object **uniek** maakt — een `aria-label`, een testid, de inhoud — nooit op zijn tag of type, en tel eerst: `querySelectorAll(<selector>).length` hoort **1** te zijn vóór je er een eigenschap van afleest. Dezelfde vorm bij een **scope**: `git status --porcelain` rapporteert een nieuwe map als één regel, dus een `.tsx$`-filter erover gooit élk bestand erin weg — 18 bestanden waar er 23 waren, en het enige foute token zat in de vijf die wegvielen. Gebruik `-uall`, naast `git diff --name-only origin/main...HEAD`.

*Een grep-treffer is niet de entry.* Een `grep -n` geeft je één regel; de `Status` van dat item staat daarbuiten. Gemeten over `HANDOFF.md`, `BACKLOG.md` en `LEARNINGS.md` (79 entries, 2026-08-26): de statusregel staat gemiddeld 4,4–4,9 regels onder zijn kop en in de staart tot **+11**, dus een kale grep toont hem bij **geen enkele** entry. Zo werd een `resolved` HANDOFF-item als openstaand gepresenteerd terwijl de tegenproef ín dat item de bewering weersprak — en het tweede signaal lag er ongelezen: de SessionStart-hook toont alléén open items, en dit stond er niet bij. Een groter venster is geen fix maar een nieuwe proxy: `grep -A7` dekt het gemiddelde en mist de staart. Anker op de entry:
...
Geldt voor elke gelaagde lus-file: lees een entry tot en met zijn status vóór je hem citeert. Elk stuk van dat commando betaalt voor een val die op 2026-08-27 in drie opeenvolgende eigen pogingen toesloeg — alle drie stil, alle drie met een plausibel getal. `^## [0-9]{4}-` in plaats van `^## `, want het **format-voorbeeld in de bestandskop** draagt zelf een `## YYYY-MM-DD`-kop mét een `Status: open` en wordt anders meegeteld — het meetbereik bevat de meting. `if(h!="")`, want dat voorbeeld staat vóór de eerste echte kop en levert anders een treffer met een lege titel. En `$3==s` in plaats van `grep -w open`, want een grep matcht de hele regel en dus ook het woord "open" in een **titel** — hier de entry *"resolved item als open gepresenteerd"*, die daardoor als openstaand terugkwam terwijl hij `promoted` is. Filter op het veld, niet op de regel.
...
*Een verwachtingswaarde is geen meting.* Een vuistregel uit de literatuur, een typische waarde uit ervaring of een aggregaat dat logisch oogt is een **hypothese**, nooit het bewijs dat een empirische vraag afsluit. Bestaat de meetbare as — een log, een opname, een teller die je kunt uitlezen — dan is díe het antwoord en is de verwachting hoogstens de reden om te gaan meten. Herkenningsteken: wijkt het getal af met precies een ronde factor (×2, ×½, ×60), dan is dat een tel- of eenheidsfout die je meet in plaats van verklaart, en de kant waarop hij valt beslis je nooit uit plausibiliteit. Kun je niet meten, dan is de uitkomst een hypothese met het meetpad erbij — geen conclusie met een tabel eronder. Het schaadt het meest in een **keuzevraag**: een schatting in een `AskUserQuestion`-optie wordt de grond waarop de gebruiker beslist, en is daar niet als schatting herkenbaar. Gemeten op 2026-08-26: de optie "Alle drie (aanbevolen)" droeg "−3 500 chars uit consolidatie"; de meting gaf **−428**, een factor 8 — en die meting was op dat moment al mogelijk met de blokmeting die twee stappen eerder gedraaid was. Zet in een optie dus wat je gemeten hebt, of markeer het getal expliciet als schatting. En kantelt een aanname onder een optie nadat de gebruiker gekozen heeft, dan is de opbrengst corrigeren niet genoeg — bied de keuze opnieuw aan.

*Je zekerheid moet dekken wat je werkelijk deed.* Een melding draagt impliciet dat er een tool-call onder zit. Twee vormen waarin dat op 2026-08-26 niet klopte. **Een verklaring die je niet toetste:** na drie gefaalde `figma_execute`-calls noemde ik het `·`-teken als oorzaak ("overleeft de bridge-encoding niet") en meldde dat als vaststaand; de controle-test gaf vijf keer groen — `'02 · Schermen'.indexOf('02 · ')===0` is gewoon `true`. De echte oorzaak: Figma vernoemt tekstnodes naar hun inhoud, dus de sectiekop-TEXT heet óók `02 · Schermen` en heeft geen `findAll`. **Een handeling die je niet uitvoerde:** ik schreef "Ik heb hem toegevoegd aan de LEARNINGS-entry" zonder één call die dat deed — `grep -n "porcelain" LEARNINGS.md` gaf nul treffers. Herkenningsteken voor beide: een bewering die in dezelfde adem ontstaat én wordt afgevinkt, zonder call ertussen. Zeg "vermoedelijk" tot je het getoetst hebt, en vink een schrijfactie pas af ná de call die hem uitvoert — van buitenaf is niet te zien welke "gedaan"-melding op bewijs rust en welke op een voornemen.
