...
- **Volgende zet:** Kiezen uit drie: (1) RowTrack uit de pnpm-workspace halen (`!apps/rowtrack` in `pnpm-workspace.yaml`, eigen lockfile en install, eigen hoisted layout) — gangbaar voor een RN-app in een gemengde monorepo, kost een tweede install-stap en turbo-aanpassing; (2) de phantom deps expliciet declareren in `apps/rowtrack/package.json`, te beginnen met `@expo/metro-runtime`, en de rest opsporen door ze tegen te komen; (3) laten zoals het is en in de web-apps geen dependencies met een React-peer meer toevoegen. Optie 3 blokkeert nu de chart-library in cashflow (fase 4). Let op bij elke wissel tussen layouts: `pnpm install` vraagt interactief om bevestiging (`CI=true` omzeilt dat) en herstelt de `.bin`-links van workspace-packages niet — enkel een volledige `rm -rf node_modules apps/*/node_modules packages/*/node_modules` gevolgd door een schone install brengt alles terug.
- **Status:** open
