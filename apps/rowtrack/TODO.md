...
- [ ] **`accent @0.20` selectie-fill** — nu hardcoded in `apps/rowtrack/components/Chip.tsx:42`, `apps/rowtrack/components/GoalSegments.tsx:154` (+ TabItem). `accent.muted` = 0.12 is de dichtstbijzijnde. `[HANDOFF · debt]` `[code-TODO]`
- [ ] **`accent.subtle-10` (0.10) pill-bg** — inline rgba in `apps/rowtrack/components/workout/ActivePhase.tsx:679`; raakt ook de portrait- én landscape doel-pill. `accent.subtle`=0.06 / `accent.muted`=0.12. `[code-TODO]` `[briefing 2026-07-08-feature-landscape-metrics]`
- [ ] **`fontSize.114`** — hero-cijfer; grootste bestaande token = 96. `apps/rowtrack/components/workout/ActivePhase.tsx:748`. `[code-TODO]`
- [ ] **Goud-token voor `#FFD700`** — titelkleur bij goal bereikt in `apps/rowtrack/components/MotivationalToast.tsx:200` (dichtstbijzijnde: `achievement.default` #E8DCC4), of bewust hardcoded laten met `// TODO`. `[PR#66]` `[audit laag-flags]`
- [ ] **Bredere ontbrekende primitieven** (uit `2026-07-09-laag-flags.md`): status/error, scrim/scrimStrong, alpha/opacity/fontWeight-primitieven. `[audit laag-flags]`
...
- [ ] **Hero-font active workout** — Albert Sans Bold (alle 6 Figma-frames, 114) vs Source Serif (huidige code + eerdere bewuste keuze). Bepaalt of er nieuwe font-weights in de pipeline moeten. `[briefing 2026-07-13-active-workout-redesign-snapshot]`
...
- [ ] **Re-triage `briefings/2026-07-04-audit-design-vs-code.md`** — 214-item beslisdocument, 0 afgevinkt, maar de headline (token-pipeline `[object Object]` in `constants/colors.ts`) is **al opgelost** (geverifieerd 14-jul: niet meer aanwezig). Het hele doc verdient een herweging i.p.v. blind afwerken. `[audit 2026-07-04]`
