...
Dit bestand is de **rauwe vangst** van momenten waarop een skill of werkprincipe faalde. Het staat los van CLAUDE.md: CLAUDE.md blijft schone instructie, LEARNINGS.md is de staging-area waaruit bewezen regels later naar de juiste laag **promoveren** — een `SKILL.md`-stap, of de CLAUDE.md van de globale, klant- of projectlaag.
...
- `promoted` — de regel is gehard naar de juiste laag: een stap in `.claude/skills/{naam}/SKILL.md`, een CLAUDE.md-laag (globaal / klant / project), of een code-guard.
...
Elke entry staat onder een laag-header — `# Skill` (de fix hardt in `.claude/skills/{naam}/SKILL.md`), `# Globaal`, `# Klant — {naam}` of `# Project — {app}` (de fix hardt in de CLAUDE.md van die laag) — en heeft deze vorm:
